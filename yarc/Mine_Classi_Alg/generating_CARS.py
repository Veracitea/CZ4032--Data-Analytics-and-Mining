...
def generateCARs(transactionDB, support=1, confidence=50, maxlen=10, **kwargs):
    ...
    The output of the code is passed onto CARlist function below to convert into list of CARs
    ...
    support : float
        ...
        absolute the minimum support if negative

    confidence : float
        minimum confidence in percents if positive
        absolute the minimum confidence if negative
...
def top_rules(transactions,
              ...
              total_timeout=100.,
              max_iterations=30):
    """Function for finding the best k (target_rule_count)
