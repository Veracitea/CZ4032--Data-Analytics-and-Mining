...
from ..Structure import ClassAssocationRule, Antecedent, Consequent
...
class M2Classi():
    ...
    def calculate_default_class_properties(self, pre):
        """This function is used for calculating
        default class support and confidence
        """
        default_class = pre.default_class
        class_distribution = Counter([ value for _, value in self.y])

        pre.default_class_support = class_distribution[default_class] / len(self.y)
        pre.default_class_confidence = class_distribution[default_class] / len(self.y)

        default_rule_ant = Antecedent({})
        default_rule_conseq = Consequent(pre.default_class_attribute, pre.default_class)

        pre.default_rule = ClassAssocationRule(
            default_rule_ant,
            default_rule_conseq,
            pre.default_class_support,
            pre.default_class_confidence
        )
    ...
    def build(self):
        ...
        pre = Predictor()
        pre.rules = self.classifier
        pre.default_class = self.default_class
        self.calculate_default_class_properties(pre)
